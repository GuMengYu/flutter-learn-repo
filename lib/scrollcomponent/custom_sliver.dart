...
class CustomSliver extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(title: Text("Custom Sliver")),
...
class SliverFlexibleHeader extends SingleChildRenderObjectWidget {
  SliverFlexibleHeader(
      {Key? key, this.visiableExtent = 0, required Widget child})
      : super(key: key, child: child);

  final double visiableExtent;

  @override
  RenderObject createRenderObject(BuildContext context) {
    return _FlexibleHeaderRenderSliver(visiableExtent);
  }

  @override
  void updateRenderObject(
      BuildContext context, _FlexibleHeaderRenderSliver renderObject) {
    renderObject._visibleExtent = visiableExtent;
  }
}

class _FlexibleHeaderRenderSliver extends RenderSliverSingleBoxAdapter {
  _FlexibleHeaderRenderSliver(double visiableExtent)
      : _visibleExtent = visiableExtent;
